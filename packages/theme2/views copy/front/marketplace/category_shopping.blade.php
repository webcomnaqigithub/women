@extends('base')
@section('title') Category Shopping | بازار - Bazaar @endsection
...
<div class="bigen-container-shop-data ">
	...
	<div class="breadcrumbs d-flex flex-row align-items-center ">
		<ul>
			...
			<li class="active"><a><i class="fa fa-angle-left" aria-hidden="true"></i>ملابس</a></li>
		</ul>
	</div>
	</div>
	<!--  search  -->
	...
	<div class="Shop-by  ">
		...
		<div class="search-input">
			<div class="input-icon">
				<input type="text" name="sk" class="input-icon-radius form-control" value="{{$request->sk}}" placeholder="{{aitrans('embroidery handbag', [], 'client')}}" form="category_shopping">
				{{-- <button class="btn clear-input cursor-pointer round-20 mt-cust f-left">
				<i class="fa fa-times-circle"></i>
				</button> --}}
				<button type="submit" class="btn btn-danger cursor-pointer round-20 mt-cust f-left" form="category_shopping">
		...
		<div class="col-lg-12 d-flex-space-between">
			...
			<div class="Shop-by-product">
				<label class="label-chack">
					<input type="radio" name="categoryId" value="*" class="card-input-element">
				...
				@foreach ($cat_list as $item)
					<label class="label-chack">
						<input type="radio" name="categoryId" value="{{$item->id}}" id="{{$item->code}}" @if ($cat->code == $item->code) checked @endif class="card-input-element catlist">
							<label class="panel panel-default card-input" for="{{$item->code}}">
								<a href="{{route('front.product.category_shopping', ['locale'=>app()->getLocale(), 'code'=>$item->code])}}"  @if ($cat->code == $item->code) style="color:#fff" @else style="color:#000" @endif >
									{{$item->label}}
								</a>
						</label>
					</label>
			...
			<ul class="product_sorting">
				<li>
				<span class="type_sorting_text">{{aitrans('sort by', [], 'client')}}</span>
				<i class="fa fa-angle-down"></i>
				<ul class="sorting_type">
					<li class="type_sorting_btn"
						data-isotope-option="{ &quot;sortBy&quot;: &quot;original-order&quot; }">
						<span> {{aitrans('Elements', [], 'client')}}</span>
					</li>
					<li class="type_sorting_btn"
						data-isotope-option="{ &quot;sortBy&quot;: &quot;price&quot; }">
						<span>{{aitrans('Price', [], 'client')}}</span>
					</li>
					<li class="type_sorting_btn"
						data-isotope-option="{ &quot;sortBy&quot;: &quot;name&quot; }">
						<span> {{aitrans('Product', [], 'client')}}</span>
					</li>
				</ul>
				</li>
			</ul>
	...
	<div class="row">
		<div class="filter-option  d-none ">
			<form action="{{airoute('front.product.category_shopping', ['code'=>$request->code])}}" method="GET" id="category_shopping">
				<div class="sidebar">
					<!-- Elements -->
					<div class="row ">
					<div class="sidebar_section mt-2">
						<ul class="checkboxes">
							<li><i class="fa fa-square-o" aria-hidden="true"></i><span>{{aitrans('Product Rating', [], 'client')}}</span></li>
		...
		<div class="filter-content col-md-12">
			<div class="row mt-3">
				@foreach ($productItems as $productItem)
						...
						<div class="store-categories col-md-3 mb-5">
