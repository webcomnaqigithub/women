...
<?php foreach( $prices as $priceItem ) : ?>
	<?php
		...
		$currency = $this->translate( 'client', $priceItem->getCurrencyId() );
	?>
	...
	<div class="price-item <?= $enc->attr( $priceItem->getType() ) ?>" itemscope itemprop="priceSpecification" itemtype="http://schema.org/PriceSpecification">
		...
		<span class="value">
			<?php if( $priceItem->getValue() !== null ) : ?>
				<?= $enc->html( sprintf( $format['value'], $priceItem->getValue() , $currency ), $enc::TRUST ) ?>
			<?php else : ?>
		...
		<?php if( $this->get( 'all' ) || $priceItem->getCosts() > 0 ) : ?>
			<span class="costs">
				<?= $enc->html( sprintf( $format['costs'],  $priceItem->getCosts() , $currency ), $enc::TRUST ) ?>
			</span>
		<?php endif ?>
