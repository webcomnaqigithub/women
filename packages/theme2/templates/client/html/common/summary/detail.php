...
<div>
			...
			<div class="row g-0 product-item <?= ( isset( $errors['product'][$position] ) ? 'error' : '' ) ?>">
				...
				<div class="col-8 col-md-6">
					<div class="row g-0">
						...
						<div class="unitprice col-4"><?= $enc->html( sprintf( $priceFormat, $product->getPrice()->getValue(), $priceCurrency ) ) ?></div>
						<div class="price col-3"><?= $enc->html( sprintf( $priceFormat, $product->getPrice()->getValue() * $product->getQuantity(), $priceCurrency ) ) ?></div>
						<?php if( $modify ) : ?>
	...
	<?php foreach( $this->summaryBasket->getService( 'delivery' ) as $service ) : ?>
		<?php if( $service->getPrice()->getValue() > 0 ) : $priceItem = $service->getPrice() ?>
			<?php $price = $enc->html( sprintf( $priceFormat, $priceItem->getValue(), $priceItem->getCurrencyId() ) ) ?>
		...
		<div class="subtotal row g-0">
			<div class="col-8 col-md-6 offset-4 offset-md-6">
				<div class="row g-0">
					...
					<div class="price col-3"><?= $enc->html( sprintf( $priceFormat, $this->summaryBasket->getPrice()->getValue(), $priceCurrency ) ) ?></div>
					<?php if( $modify ) : ?>
						<div class="action col-1"></div>
					<?php endif ?>
				</div>
		...
		<div class="delivery row g-0">
			<div class="col-8 col-md-6 offset-4 offset-md-6">
				<div class="row g-0">
					...
					<div class="price col-3"><?= $enc->html( sprintf( $priceFormat, $costs, $priceCurrency ) ) ?></div>
					<?php if( $modify ) : ?>
					<div class="action col-1"></div>
					<?php endif ?>
				</div>
		...
		<div class="total row g-0">
			<div class="col-8 col-md-6 offset-4 offset-md-6">
				<div class="row g-0 price-total">
					...
					<div class="price col-3"><?= $enc->html( sprintf( $priceFormat, $this->summaryBasket->getPrice()->getValue() + $this->summaryBasket->getPrice()->getCosts(), $priceCurrency ) ) ?></div>
					<?php if( $modify ) : ?>
						<div class="action col-1"></div>
					<?php endif ?>
				</div>
		...
		<?php foreach( $map as $taxRate => $priceItem ) : ?>
			<?php if( ( $taxValue = $priceItem->getTaxValue() ) > 0 ) : ?>
				<div class="tax row g-0">
					<div class="col-8 col-md-6 offset-4 offset-md-6">
						<div class="row g-0">
							<div class="col-8"><?= $enc->html( sprintf( $priceTaxflag ? $taxFormatIncl : $taxFormatExcl, $taxRate, $this->translate( 'client/code', $taxName ) ) ) ?></div>
							<div class="price col-3"><?= $enc->html( sprintf( $priceFormat, $taxValue, $priceCurrency ) ) ?></div>
							<?php if( $modify ) : ?>
								<div class="action col-1"></div>
							<?php endif ?>
						</div>
