...
<?php if (isset($this->profileItem)) : ?>
	<?php if (auth()->user()->merchant == true) : ?>
		<div class="bigen-container-shop-data mb-3 ">
			...
			<div class="container ">
				<div>
					<img src="<?= $enc->attr($this->content('front/images/Cover-Full.png')) ?>" alt="Avatar" loading="lazy" class="image" style="width:100%">
					<div class="">
						<!-- <div class="text">
						<label class="image-upload" for="profile_image">
						...
						<input type="file" id="profile_image" style="display: none;" onchange="loadFile(event)">
					</div>
				</div>
					...
					<div class="col-md-10 tab-div">
						<div class="tab-content" id="myTabContent">
							...
							<div class="tab-pane fade " id="users" role="tabpanel" aria-labelledby="users-tab">
								<form action="<?= airoute('front.mrchnt.updateProfile', ['locale' => 'en']) ?>" method="POST" enctype="multipart/form-data">
									<?= $this->csrf()->formfield(); ?>
									...
									<div class="row mt-3">
										...
										<div class="col-md-12 my-1 mt-3">
											...
											<input type="text" class="form-control" name="store_label" value="<?= auth()->user()->store_label ?>" placeholder="اسم المتجر">
											<div class="text-danger mt-1"><?= $this->get('errors')->first('store_label') ?></div>
										</div>
										<div class="col-md-12 my-1 mt-3">
											<label><?= $enc->html($this->translate('client', 'Brief')) ?></label>
											<textarea rows="3" type="text" name="summary" class="form-control" placeholder="<?= $enc->html($this->translate('client', 'Abstract')) ?>"><?= auth()->user()->summary ?></textarea>
												...
												<div class="container" style="margin-top: 20px;">
													<div class="row">
														<div class="col-sm-2 imgUp">
															<div class="imagePreview"></div>
															<label class="btn btn-primary">
																<span><?= $enc->html($this->translate('client', 'upload photo')) ?></span><input type="file" name="summary_pics[0]" class="uploadFile img" style="width:143px;height:20px;overflow: hidden;">
															</label>
														</div>
							...
							<div class="tab-pane fade show active " id="Orders" role="tabpanel" aria-labelledby="home-tab">
								...
								<div class="tab_orders">
									<div class="row">
										<div class="col">
											<div class="tabs_container">
												<ul class="tabs d-flex align-items-md-center justify-content-center" id="mytab" role="tablist">
													<li class="nav-item" >
														<a class="nav-link li-radius-top d-flex  active" data-toggle="tab" href="#pending" role="tab" aria-controls="mytab" aria-selected="false">
															<span>قيد الإنتظار</span>
														</a>
													</li>
													<li class="nav-item" >
														<a class="nav-link li-radius-top d-flex " data-toggle="tab" href="#proccess" role="tab" aria-controls="mytab" aria-selected="false">
															<span>حالية</span>
														</a>
													</li>
													<li class="nav-item" >
														<a class="nav-link li-radius-top d-flex " data-toggle="tab" href="#complete" role="tab" aria-controls="mytab" aria-selected="false">
															<span>مكتملة</span>
														</a>
													</li>
												</ul>
									...
									<div class="row mx-1 mt-n13">
										<?php if (!$this->get('historyItems', map())->isEmpty()) : ?>
											<div class="tab_container active" id="pending">
												<?php foreach ($this->get('historyItems', []) as $id => $orderItem) : ?>
											...
											<div class="tab_container" id="proccess">
												<?php foreach ($this->get('historyItems', []) as $id => $orderItem) : ?>
											...
											<div class="tab_container" id="complete">
												<?php foreach ($this->get('historyItems', []) as $id => $orderItem) : ?>
							...
							<div class="tab-pane fade " id="users" role="tabpanel" aria-labelledby="users-tab">
								<form method="POST" action="<?= airoute('front.usr.updateProfile', ['locale' => 'en']) ?>" method="POST" enctype="multipart/form-data">
									...
									<div class="row mt-3">
										<div class="col-md-6  my-1">
											<label><?= $enc->html($this->translate('client', 'Full Name')) ?></label>
											<input type="text" class="form-control" name="name" placeholder=" <?= $enc->html($this->translate('client', 'Full Name')) ?>" value="<?= auth()->user()->name ?>">
											<div class="text-danger mt-1"><?= $this->get('errors')->first('name') ?></div>
										</div>
										<div class="col-md-6  my-1">
											<label><?= $enc->html($this->translate('client', 'E-mail')) ?></label>
											<input type="text" class="form-control" disabled placeholder=" <?= $enc->html($this->translate('client', 'E-mail')) ?>" value="<?= auth()->user()->email ?>">
										</div>
										<div class="col-md-6  my-1">
											<label><?= $enc->html($this->translate('client', 'Mobile number')) ?></label>
											<div>
										...
										<div class="col-md-12 my-1 ">
											<label><?= $enc->html($this->translate('client', 'address')) ?></label>
											<input type="text" class="form-control" name="address1" placeholder="<?= $enc->html($this->translate('client', 'Example: Next to Palestine School')) ?>" value="<?= auth()->user()->address1 ?>">
											<div class="text-danger mt-1"><?= $this->get('errors')->first('address1') ?></div>
										</div>
									</div>
								</form>
