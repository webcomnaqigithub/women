		...
		<div class="row  ">
			...
			<div class="col-md-10 tab-div">
				...
				<form method="POST" action="<?= route('front.store_address', ['locale' => app()->getLocale()]) ?>" id="address-form">
					...
					<input type="hidden" name="addrid" value="<?= $this->get('customer')->getId() ?>">
					<div class="row Register-input mt-2">
						<div class="col-md-6 my-1">
							<label> <?= $enc->html($this->translate('client', 'Full Name')) ?></label>
							<input type="text" name="firstname" class="form-control" placeholder="<?= $enc->html($this->translate('client', 'Full Name')) ?>" value="<?= $this->get('customer')->getFirstName()?>">
							<div class="text-danger"><?= $this->get('errors')->first('firstname')?></div>
						</div>
						<div class="col-md-6 my-1">
							<label><?= $enc->html($this->translate('client', 'Mobile number')) ?></label>
							<div>
								<input type="tel" class="form-control input-tel" name="telephone" value="<?= $this->get('customer')->getTelephone()?>">
						...
						<div class="col-md-6  my-1">
							...
							<select class="form-control select" name="countryid">
								...
								<?php foreach( $this->get( 'addressCountries', [] ) as $countryId ) : ?>
									<option value="<?= $enc->attr( $countryId ) ?>"  <?php if($countryId == $this->get('customer')->getCountryId()):?> selected <?php endif?>>
						...
						<div class="col-md-6  my-1">
							...
							<select class="form-control select" name="city">
								...
								<option value="غزة" <?php if($this->get('customer')->getCity() == 'غزة'):?> selected <?php endif?>>غزة</option>
							</select>
							<div class="text-danger"><?= $this->get('errors')->first('city')?></div>
						</div>
						<div class="col-md-6  my-1">
							<label><?= $enc->html($this->translate('client', 'street')) ?></label>
							<input type="text" class="form-control" name="address1" placeholder=" <?= $enc->html($this->translate('client', 'street')) ?>" value="<?= $this->get('customer')->getAddress1()?>">
							<div class="text-danger"><?= $this->get('errors')->first('address1')?></div>
						</div>
						<div class="col-md-6  my-1">
							<label><?= $enc->html($this->translate('client', 'ZIP code')) ?></label>
							<input type="text" class="form-control" name="postal" placeholder="مثال : P860" value="<?= $this->get('customer')->getPostal()?>">
